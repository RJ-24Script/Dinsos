...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const subBtn = document.querySelector('[data-toggle="submenu-profil"]');
  const subMenu = document.getElementById('submenu-profil');
  subBtn?.addEventListener('click', () => subMenu.classList.toggle('hidden'));

  // Dropdown profil (desktop by JS fallback + mobile manual)
  ...
  btnProfil?.addEventListener('click', (e) => {
    ...
    const now = !menuProfil.classList.contains('hidden');
    btnProfil.setAttribute('aria-expanded', String(now));
  });
  document.addEventListener('click', (e) => {
    if (!menuProfil?.contains(e.target) && !btnProfil?.contains(e.target)) {
